...
# Will be set properly in register()
_lib_path = None


def _get_addon_lib_path():
    """Get the path to the bundled lib folder, handling zip installs."""
    # Primary method: use __file__
    addon_dir = os.path.dirname(os.path.abspath(__file__))
    lib_path = os.path.join(addon_dir, "lib")

    # Verify the path exists and contains reportlab
    if os.path.isdir(lib_path) and os.path.isdir(os.path.join(lib_path, "reportlab")):
        return lib_path

    # Fallback: try to find via bpy.utils for Blender 5.0+ compatibility
    try:
        import bpy
        # Get addon preferences path - works even when installed from zip
        for mod in bpy.context.preferences.addons:
            if mod.module == "pipe_template_generator":
                # Get the actual addon module to find its path
                import importlib
                addon_module = importlib.import_module("pipe_template_generator")
                if hasattr(addon_module, "__file__") and addon_module.__file__:
                    addon_dir = os.path.dirname(os.path.abspath(addon_module.__file__))
                    lib_path = os.path.join(addon_dir, "lib")
    ...
    # Last resort: return the originally computed path
    return lib_path


def _ensure_lib_path():
    """Ensure the bundled lib folder is in sys.path for reportlab imports."""
    global _lib_path

    # Compute path if not already set
    if _lib_path is None:
        _lib_path = _get_addon_lib_path()

    # Add to sys.path if not present
    if _lib_path and _lib_path not in sys.path:
        sys.path.insert(0, _lib_path)

    # Force reimport of reportlab if it was previously imported incorrectly
    # This handles cases where Python cached a failed import
    if "reportlab" in sys.modules:
        # Check if the cached module is from our lib path
        reportlab_mod = sys.modules.get("reportlab")
        if reportlab_mod and hasattr(reportlab_mod, "__file__"):
            if reportlab_mod.__file__ and _lib_path not in reportlab_mod.__file__:
                # Remove stale cached imports to force fresh import from our lib
                modules_to_remove = [key for key in sys.modules if key.startswith("reportlab")]
...
class PIPE_OT_GenerateTemplate(Operator):
    ...
    def generate_pdf(self, props, uv_data, boundary_data):
        ...
        try:
            ...
            # Provide diagnostic info for troubleshooting
            lib_info = f"Lib path: {_lib_path}, exists: {os.path.isdir(_lib_path) if _lib_path else 'N/A'}"
            raise Exception(
                f"reportlab import failed: {e}. {lib_info}. "
                f"Try reinstalling the addon or ensure the lib folder is present."
            )
...
def register():
    # Initialize the bundled library path early
    global _lib_path
    _lib_path = _get_addon_lib_path()
    _ensure_lib_path()
